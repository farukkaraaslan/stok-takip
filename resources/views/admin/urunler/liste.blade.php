            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        <a href="{{ route('urun-ekle') }}" class="btn btn-success"><i class="fa fa-plus"></i> Ürün Ekle</a>
                            ...
                            <tbody>
                                @foreach ($urunler as $urun)
                                    <tr>
                                        <td>{{ $urun->id }}</td>
                                        <td>{{ $urun->adi }}</td>
                                        <td>{{ $urun->getCategory->adi }}</td>
                                        <td>{{ $urun->adet }}</td>
                                        <td>{{ $urun->fiyat }}</td>
                                        <td>{{ $urun->created_at == null ? $urun->created_at : \Carbon\Carbon::parse($urun->created_at)->format('d.m.Y H:i:s') }}
                                        </td>
                                        <td width="10%">
                                            <form action="{{ route('urun-delete', $urun->id) }}" method="Post">
                                                <a href="{{ route('urun-duzenle', $urun->id) }}" title="Düzenle"
